...
- [ ] Migration file is created in `src/db/migrations/`
- [ ] `npm run typecheck` passes
- [ ] `npm run lint` passes

#### Manual Verification:
- [ ] Review generated migration SQL for correctness
- [ ] Verify JSONB columns and foreign keys are properly defined
...
#### Manual Verification:
- [ ] Code review confirms proper user scoping in all methods
- [ ] Upsert pattern correctly updates existing records
...
#### Manual Verification:
- [ ] POST `/api/projections/save` successfully saves a projection
- [ ] GET `/api/projections/[planId]` retrieves the saved projection
- [ ] POST `/api/projections/calculate` with `planId` auto-saves the result
- [ ] Unauthorized access returns 401
- [ ] Accessing another user's plan returns 404
...
#### Manual Verification:
- [ ] GET `/api/projections/[planId]/staleness` returns `isStale: false` for fresh projection
- [ ] After modifying financial snapshot, staleness check returns `isStale: true`
- [ ] `changedFields` array correctly identifies what changed
...
#### Manual Verification:
- [ ] Table `projection_results` exists in Supabase
- [ ] RLS is enabled on the table
- [ ] RLS policy exists and matches expected pattern
- [ ] Complete flow works: calculate → save → retrieve → staleness check
