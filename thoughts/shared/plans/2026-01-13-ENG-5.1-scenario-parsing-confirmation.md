...
- [ ] API returns 400 for empty query
- [ ] "What if I retire at 65?" returns `{ overrides: { retirementAge: 65 }, ... }`
- [ ] "What if I get 7% returns?" returns `{ overrides: { expectedReturn: 0.07 }, ... }`
- [ ] "Random text" returns low confidence and empty overrides
...
- [ ] New component exists: `src/components/scenarios/ScenarioConfirmation.tsx`

#### Manual Verification:
- [ ] Dialog displays parsed parameters with confidence colors
- [ ] Green check for high confidence (≥80%)
- [ ] Yellow warning for medium confidence (50-79%)
- [ ] Red X for low confidence (<50%)
- [ ] Clicking a value enables inline editing
- [ ] Cancel button closes dialog
- [ ] Apply Scenario button triggers onConfirm callback
...
- [ ] TypeScript compiles without errors: `npm run typecheck`
- [ ] Linting passes: `npm run lint`
- [ ] New component exists: `src/components/scenarios/ScenarioInput.tsx`

#### Manual Verification:
- [ ] Text input accepts natural language queries
- [ ] Enter key triggers analysis
- [ ] Loading state shows while parsing
- [ ] Error message displays for failed parses
- [ ] Confirmation dialog opens on successful parse
- [ ] Apply triggers onApply callback with new assumptions
- [ ] Cancel clears the parsed scenario
- [ ] Input clears after successful apply
...
#### Manual Verification:
- [ ] ScenarioInput appears on plans page above the grid
- [ ] Typing "What if I retire at 65?" and clicking Analyze opens confirmation
- [ ] Clicking "Apply Scenario" updates the projection
- [ ] The chart and metrics reflect the new retirement age
- [ ] The assumptions panel on the right shows the updated value
- [ ] The scenario input is disabled while projection is loading
- [ ] Works correctly on mobile view
